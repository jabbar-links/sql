...
SELECT 'Highest Sales' AS sales_type, market_date, total_sales
FROM ranked_sales
WHERE highest_rank = 1
UNION

SELECT
  'Lowest Sales' AS sales_type, market_date, total_sales
FROM ranked_sales
WHERE lowest_rank = 1;
...
COUNT(DISTINCT c.customer_id) * 5 * vi.original_price AS possible_revenue
